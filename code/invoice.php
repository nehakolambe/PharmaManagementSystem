...
$cust_age=$_POST['age'];
$cust_sex=$_POST['sex'];
$cust_pa=$_POST['postal_address'];		
$cust_phn=$_POST['phone'];
$cust_drug=$_POST['drug_name'];
$cust_str=$_POST['strength'];
$cust_dose=$_POST['dose'];
$cust_qnt=$_POST['quantity'];
...
$presId=mysqli_fetch_array($getPresid);
	...
	// 			VALUES('$presId','$cust_id','$cust_name','$cust_age','$cust_sex','$cust_pa','$invoiceNo','$cust_phn')  ");

	$sqlP=mysqli_query($con, "INSERT INTO prescription(prescription_id,customer_id,customer_name,age,sex,postal_address,invoice_id,phone)
				VALUES($presIdd,$cust_id,'$cust_name','$cust_age','$cust_sex','$cust_pa','$invoiceNo','$cust_phn')  ");
	...
	$sqlI=mysqli_query($con,"INSERT INTO invoice(invoice_id, customer_name,served_by,status)
				VALUES('$invoiceNo','$cust_name','$user','Pending') ");			
						
	$getDetails=mysqli_query($con,"SELECT * FROM tempprescri WHERE customer_id='$cust_id'");					
...
while($item1=mysqli_fetch_array($getDetails))
		...
		//	echo $invoiceNo."details".$details['stock_id']."-".$details['cost']."-".$item1['quantity'];
			...
			$tot=array_sum($count);
			$file=fopen("receipts/docs/".$cust_id.".txt", "a+");
...
while($item12=mysqli_fetch_array($getDetails))
		...
		$sqlD=mysqli_query($con,"DELETE FROM tempprescri WHERE customer_id='$cust_id' ");
				
	
	// select all from temp prescri where cust/Id =$cust_id
